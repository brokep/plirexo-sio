...
avdmanager create avd --device "pixel_3" --tag "google_apis" --package "system-images;android-30;google_apis;x86_64" --abi "x86_64" --name "generic"

sed -i "s/hw.gps =.*/hw.gps = no/g" $HOME/.android/avd/generic.avd/config.ini
sed -i "s/hw.dPad =.*/hw.dPad = yes/g" $HOME/.android/avd/generic.avd/config.ini
sed -i "s/hw.cpu.ncore =.*/hw.cpu.ncore = 4/g" $HOME/.android/avd/generic.avd/config.ini
sed -i "s/hw.keyboard =.*/hw.keyboard = yes/g" $HOME/.android/avd/generic.avd/config.ini
sed -i "s/hw.gpu.mode =.*/hw.gpu.mode = host/g" $HOME/.android/avd/generic.avd/config.ini
sed -i "s/hw.audioInput =.*/hw.audioInput = no/g" $HOME/.android/avd/generic.avd/config.ini
sed -i "s/hw.audioOutput =.*/hw.audioOutput = no/g" $HOME/.android/avd/generic.avd/config.ini
sed -i "s/hw.gpu.enabled =.*/hw.gpu.enabled = yes/g" $HOME/.android/avd/generic.avd/config.ini
sed -i "s/fastboot.forceFastBoot =.*/fastboot.forceFastBoot = no/g" $HOME/.android/avd/generic.avd/config.ini
sed -i "s/fastboot.forceColdBoot =.*/fastboot.forceColdBoot = yes/g" $HOME/.android/avd/generic.avd/config.ini

# sed -i "s/hw.gps =.*/hw.gps = true/g" $HOME/.android/avd/generic.avd/hardware-qemu.ini
# sed -i "s/hw.dPad =.*/hw.dPad = true/g" $HOME/.android/avd/generic.avd/hardware-qemu.ini
# sed -i "s/hw.cpu.ncore =.*/hw.cpu.ncore = 4/g" $HOME/.android/avd/generic.avd/hardware-qemu.ini
# sed -i "s/hw.keyboard =.*/hw.keyboard = true/g" $HOME/.android/avd/generic.avd/hardware-qemu.ini
# sed -i "s/hw.gpu.mode =.*/hw.gpu.mode = host/g" $HOME/.android/avd/generic.avd/hardware-qemu.ini
# sed -i "s/hw.audioInput =.*/hw.audioInput = false/g" $HOME/.android/avd/generic.avd/hardware-qemu.ini
# sed -i "s/hw.gpu.enabled =.*/hw.gpu.enabled = true/g" $HOME/.android/avd/generic.avd/hardware-qemu.ini
# sed -i "s/hw.audioOutput =.*/hw.audioOutput = false/g" $HOME/.android/avd/generic.avd/hardware-qemu.ini
# sed -i "s/fastboot.forceColdBoot =.*/fastboot.forceColdBoot = true/g" $HOME/.android/avd/generic.avd/hardware-qemu.ini
...
./emulator -no-boot-anim -crostini -no-audio -netfast -no-snapshot -accel on -verbose @generic &

adb wait-for-device

# Termux install
[ -f $BASE/apk/termux/com.termux_106.apk ] && adb install $BASE/apk/termux/com.termux_106.apk
[ -f $BASE/apk/termux/com.termux_106.apk ] && [ -f $BASE/apk/termux/com.termux.api_47.apk ] && adb install $BASE/apk/termux/com.termux.api_47.apk
[ -f $BASE/apk/termux/com.termux_106.apk ] && [ -f $BASE/apk/termux/com.termux.widget_11.apk ] && adb install $BASE/apk/termux/com.termux.widget_11.apk

# OpenVPN client install
[ -f $BASE/apk/openvpn/de.blinkt.openvpn_175.apk ] &&  adb install $BASE/apk/openvpn/de.blinkt.openvpn_175.apk

# Bitmask install 
[ -f $BASE/apk/normalInsecureFat/debug/Bitmask_debug_1.0.7.apk ] && adb install $BASE/apk/normalInsecureFat/debug/Bitmask_debug_1.0.7.apk
# [ -f $BASE/apk/normalProductionFat/release/Bitmask_release_1.0.7.apk ] && adb install $BASE/apk/normalProductionFat/release/Bitmask_release_1.0.7.apk
# [ -f $BASE/apk/customProductionFat/release/RiseupVPN_release.apk ] && adb install $BASE/apk/customProductionFat/release/RiseupVPN_release_1.0.7.apk
